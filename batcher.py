...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    self.enc_batch:
      ...
      (all OOVs represented by UNK id), padded to length of longest sequence
      in the batch
    self.enc_lens:
    ...
    self.enc_padding_mask:
      ...
      1s correspond to real tokens in enc_batch and target_batch; 0s correspond
      to padding.
    ...
    self.enc_batch_extend_vocab:
      Same as self.enc_batch, but in-article OOVs are represented by their
      temporary article OOV number.
    """
    ...
    # batch because we use dynamic_rnn for the encoder.
  ...
  def init_decoder_seq(self, example_list, hps):
    ...
    # batch (second dimension = max_dec_steps) because we do not use a dynamic
    # rnn for decoding. However I believe this is possible, or will soon be
    # possible, with Tensorflow 1.0, in which case it may be best to upgrade to
    # that.
...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, hps, single_pass):
    ...
    Args:
      ...
      when you want to run evaluation on the dev or test set).
