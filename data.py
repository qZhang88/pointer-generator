...
PAD_TOKEN = '[PAD]'  # pad encoder input, decoder input and target sequence
UNKNOWN_TOKEN = '[UNK]'  # represent out-of-vocabulary words
START_DECODING = '[START]'  # start of every decoder input sequence
STOP_DECODING = '[STOP]'  # end of untruncated target sequences


# Note:
# none of <s>, </s>, [PAD], [UNK], [START], [STOP] should appear in the vocab
# file.
...
class Vocab(object):
  ...
  def __init__(self, vocab_file, max_size):
    ...
    Args:
      vocab_file: path to the vocab file, which is assumed to contain "<word>
        <frequency>" on each line, sorted with most frequent word first. This
        code doesn't actually use the frequencies, though.
  ...
  def word2id(self, word):
    """Returns the id (integer) of a word (string). Returns [UNK] id if word is
    OOV."""
  ...
  for w in abstract_words:
    ...
    if i == unk_id:  # If w is an OOV word
      if w in article_oovs:  # If w is an in-article OOV
        vocab_idx = vocab.size() + article_oovs.index(w)  # Map to article OOV
  ...
  for i in id_list:
    try:
      ...
      assert article_oovs is not None, ("Error: model produced a word ID that "
                                        "isn't in the vocabulary. This should "
                                        "not happen in baseline "
                                        "(no pointer-generator) mode")
      article_oov_idx = i - vocab.size()
      try:
        w = article_oovs[article_oov_idx]
      except ValueError:  # i doesn't correspond to an article oov
        raise ValueError(('Error: model produced word ID %i which corresponds '
                          'to article OOV %i but this example only has %i '
                          'article OOVs') %
...
def abstract2sents(abstract):
  ...
  Args:
    abstract: string containing <s> and </s> tags for starts and ends of
      sentences
