...
def masked_softmax(e, mask):
  """Take softmax of e then apply mask and re-normalize
  Args:
    e: [batch_size, logits]
    mask: [batch_size, logits]
  """
  prob = nn_ops.softmax(e)
  prob *= mask
  masked_sums = tf.reduce_sum(prob, axis=1)
  return prob / tf.reshape(masked_sums, [-1, 1])


# Note: this function is based on tf.contrib.legacy_seq2seq_attention_decoder, which is now outdated.
# In the future, it would make more sense to write variants on the attention mechanism using the new seq2seq library
...
def attention_decoder(decoder_inputs,
  ...
  Args:
    ...
    enc_padding_mask: 2D Tensor [batch_size x attn_length] containing 1s and
    ...
    initial_state_attention: Note that this attention decoder passes each
      ...
      a zero vector. If initial_state_attention is True, we use initial_state
      to (re)calculate the previous step's context vector. We set this to
      False for train/eval mode (because we call attention_decoder once
      for all decoder steps) and True for decode mode (because we call
  ...
  Returns:
    ...
    state: The final state of the decoder. A tensor shape
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    if prev_coverage is not None:
      # reshape (batch_size, attn_length) -> (batch_size, attn_len, 1, 1)
    ...
    context_vector.set_shape([None, attn_size])  # Ensure the second shape is set.
    if initial_state_attention:  # true in decode mode
      # Re-calculate the context vector from the previous step so that we can
      # pass it through a linear layer with this step's input to get a modified
      # version of the input in decode mode, this is what updates the coverage
      # vector
