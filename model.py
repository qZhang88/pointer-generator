...
class SummarizationModel(object):
  ...
  def _calc_final_dist(self, vocab_dists, attn_dists):
    ...
    Args:
      vocab_dists: The vocabulary distributions. List length max_dec_steps of
        (batch_size, vsize) arrays. The words are in the order they appear in
        the vocabulary file.
    ...
    with tf.variable_scope('final_distribution'):
      ...
      # Concatenate some zeros to each vocabulary dist, to hold the probabilities
      # for in-article OOV words the maximum (over the batch) size of the
      # extended vocabulary
      ...
      attn_dists_projected = [tf.scatter_nd(indices, copy_dist, shape)
                              for copy_dist in attn_dists]  # list length max_dec_steps (batch_size, extended_vsize)

      # Add the vocab distributions and the copy distributions together to get
      # the final distributions final_dists is a list length max_dec_steps; each
      # entry is a tensor shape (batch_size, extended_vsize) giving the final
      # distribution for that decoder timestep Note that for decoder timesteps
      # and examples corresponding to a [PAD] token, this is junk - ignore.
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      # Our encoder is bidirectional and our decoder is unidirectional so we need to reduce the final encoder
      # hidden state to the right size to be the initial decoder hidden state
      ...
      if hps.mode in ['train', 'eval']:
        ...
        with tf.variable_scope('loss'):
          if FLAGS.pointer_gen:
            ...
            # This is fiddly; we use tf.gather_nd to pick out the probabilities
            # of the gold target words
            loss_per_step = []
            batch_nums = tf.range(0, limit=hps.batch_size)  # [batch_size]
          ...
          if hps.coverage:
            with tf.variable_scope('coverage_loss'):
              self._coverage_loss = _coverage_loss(self.attn_dists,
    ...
    if hps.mode == "decode":
      ...
      # final_dists is a singleton list, shape (batch_size, extended_vsize)
  ...
  def run_train_step(self, sess, batch):
    ...
    Returns a dictionary containing train op, summaries, loss, global_step
